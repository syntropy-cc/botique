<DOCUMENT filename="post_ideator.md">
[ROLE]
...
[TASK]
1. **Analyze the article inline**: Extract and summarize its core elements without a separate output section. Identify the title (or infer one), main thesis, detected tone, {num_insights_min}–{num_insights_max} key insights (categorized by type with strength scores and optional source quotes), major themes, and {num_keywords_min}–{num_keywords_max} keywords. Also include the main message and avoid topics for coherence.
2. **Brainstorm {num_ideas_min}–{num_ideas_max} diverse post ideas**: Each idea is a high-level concept for transforming the article into a social media post (or series). Make them varied:
   - Different platforms (e.g., {platforms_examples}).
...
[CONSTRAINTS]
- Generate exactly {num_ideas_min}–{num_ideas_max} ideas.
- Platforms: Limit to {platforms}.
- Formats: {formats}.
- Tones: Keep to {tones_word_limit} words (e.g., {tones_examples}).
- Insights: Each idea must reference {insights_per_idea_min}–{insights_per_idea_max} from your summary (use their IDs).
- Narrative Arc: High-level outline only (e.g., "Hook → Problem → Insights → CTA"); no detailed slide breakdowns.
- Estimated Slides: {estimated_slides_min}–{estimated_slides_max}; base on idea complexity.
...
- Idea Explanation: {explanation_sentences_min}–{explanation_sentences_max} sentences detailing the concept, why it's suitable for the platform/persona, how it engages the audience, and tips for development (without full structure).
- Confidence: Base on fit (high if article aligns perfectly).
- Risks: List {risks_min}–{risks_max} only if notable; otherwise empty array.
- Diversity: No two ideas can share the exact same platform + tone + objective combo.
- Length: Keep fields punchy; total JSON <{max_json_chars} chars.
...
- Coherence Elements: Suggest values based on idea; e.g., vocabulary_level as {vocabulary_levels}, formality as {formality_levels}.
  ...
  "ideas": [
    {
      ...
      "tone": "string",
      "persona": "string",
      "personality_traits": ["string (suggested traits for voice, e.g., {personality_traits_examples})"],
      "objective": "string ({objectives})",
      "angle": "string (unique spin, 1 sentence)",
      "hook": "string (<{hook_max_chars} chars)",
      "narrative_arc": "string (high-level outline, e.g., 'Hook → Problem → Solutions → CTA')",
      "vocabulary_level": "string (e.g., {vocabulary_levels})",
      "formality": "string (e.g., {formality_levels})",
      ...
      "idea_explanation": "string (detailed brainstorm: why this idea, how to develop, potential impact; {explanation_sentences_min}–{explanation_sentences_max} sentences)",
...
[VALIDATION]
Self-check: Does the JSON validate? Are ideas diverse and brainstorm-like (detailed but not structured)? Is article_context_for_idea specific to each idea? Insights referenced correctly? Coherence fields populated sensibly? If not, regenerate internally before outputting.</DOCUMENT>
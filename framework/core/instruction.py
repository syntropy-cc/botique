...
Location: framework/core/instruction.py
"""
...
class Instruction:
    ...
    def execute(
        ...
        llm_client: Any,  # LLMClient instance (from framework.llm)
        ...
        Args:
            ...
            llm_client: LLMClient instance for LLM execution
        ...
        # Note: This requires prompt_registry which will be moved to framework
        # For now, we'll handle this in the LLM client or make it optional
        if not self.prompt_id:
            # Try to register prompt if registry is available
            try:
                from ..llm.prompt_registry import get_or_register_prompt
                self.prompt_id, _ = get_or_register_prompt(
                    prompt_key=self.prompt_key,
                    template=self.prompt_template,
                )
            except ImportError:
                # Prompt registry not available, continue without registration
                pass
    ...
    def _inject_memory_context(
        ...
        Args:
            ...
            memory_context: Pre-extracted memory context (from state strategy)
